...
public class Lab {
    ...
    private final static String ORACLE_USER = "s182190";
    private final static String ORACLE_PASS = "tny395";
        ...
        try {
            ...
            try (Connection connection = DriverManager.getConnection(ORACLE_URL, ORACLE_USER, ORACLE_PASS)) {
                for (String query: QUERY_MARK)
                    try (OracleCachedRowSet oracleCachedRowSet = new OracleCachedRowSet();) {
                        oracleCachedRowSet.setCommand(query);
                        try (ObjectOutputStream objectOutputStream =
                                     new ObjectOutputStream(
                                             new FileOutputStream(oracleCachedRowSet.getTableName()))) {
